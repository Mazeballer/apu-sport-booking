...
export const Button = React.forwardRef<HTMLButtonElement, BaseProps>(
  ({ className, variant, size, asChild = false, ...props }, ref) => {
    ...
    // Dev time guard to prevent accidental anchors without asChild
    if (process.env.NODE_ENV !== "production") {
      const hasHref = Object.prototype.hasOwnProperty.call(props, "href");
      if (!asChild && hasHref) {
        delete (props as any).href;
        console.warn(
          "Button received `href` without `asChild`. Wrap a Link and pass `asChild` instead."
        );
      }
    }
